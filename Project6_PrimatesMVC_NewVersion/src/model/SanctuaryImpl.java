...
public class SanctuaryImpl implements Sanctuary{
    ...
    @Override
    public void addMonkeyFirstToSanctuary(Monkey monkey) throws IllegalArgumentException{
        if(exist(monkey)) {
            throw new IllegalArgumentException("This monkey already exists in Sanctuary, so it can't be added.");
        }
        if (isolation.isFull()) {
            throw new IllegalArgumentException("There's no more space in Sanctuary, so it can't be added.");
        }
    ...
    public void addMonkeyToIsolation(Monkey monkey) throws IllegalArgumentException{
        if (isolation.exist(monkey.getName())) {
    ...
    public String getIsolationMonkeyList() throws IllegalArgumentException{
        ...
        if (monkeys.size() == 0) {
            throw new IllegalArgumentException("There's no monkey in the Isolation!");
        }
        Collections.sort(monkeys);
        StringBuilder sb = new StringBuilder();
        for (Monkey m: monkeys) {
